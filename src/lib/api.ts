...
async function withCache<T>(
  ...
  const cached = cacheGet<T>(kind, key);

  // Start network request in parallel
  const net = fetcher()
    .then(async (data) => {
      await cachePut(kind, key, data as any, ttl);
      return data;
    })
  ...
  const timeout = new Promise<T>((res) =>
    setTimeout(async () => res((await cached) as any), 1200)
  );

  const winner: any = await Promise.race([net, timeout]);
...
export async function apiRoute(body: unknown): Promise<any> {
...
export async function apiSaveItinerary(itin: Record<string, unknown>): Promise<any> {
...
export async function apiGetItinerary(id: string): Promise<any> {
